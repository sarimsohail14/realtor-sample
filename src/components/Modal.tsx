...
export default function Modal({ property, isOpen, onClose }: ModalProps) {
  ...
  useEffect(() => {
    if (property) {
      setMessage(`Hi Ananya, I am checking in regarding "${property.name}" in ${property.location}. I would love to orchestrate a private viewing.`);
      setIsSuccess(false);
      setErrorMsg('');
    }
  }, [property]);
                ...
                <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-5 overflow-y-auto">
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      className="w-full bg-[#1D1D1F] text-white hover:bg-neutral-800 disabled:bg-neutral-300 py-3 rounded-full text-xs font-semibold transition tracking-wider uppercase"
                    >
